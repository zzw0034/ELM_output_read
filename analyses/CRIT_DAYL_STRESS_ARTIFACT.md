...
**5. 4 km SSP5-8.5 验证（2026-09-23，作业 571221，
`20260910_seus_rerun/20260923_seus_4km_cds38000/`）。** 4 个 run：
`20260917_seus_4km_fut_ssp585`、`20260915_seus_4km_fut_ssp585_DF`（36000 s），
`20260922_seus_4km_fut_ssp585{,_DF}_cds38000`（38000 s）。阈值两行是第 163/164 行
（30.8125 → 30.854°N），旧 run 复现了上文的数（1 月 Default −370、DF −1555）。

| `Default − DF` 跳变 / 起伏 | 两边 36000 | 只改 DF | 两边 38000 |
|---|---|---|---|
| 1 月 | +1184 / 11 | −372 / 11 | **−13 / 13** |
| 7 月 | +196 / 18 | +5 / 17 | +17 / 15 |
| 全年 | +314 / 12 | −30 / 12 | **+21 / 11** |

两边 38000 时 1 月的跳变落到起伏以内，全年约为起伏的 2 倍（0.5° 上也是
这个量级）。新 Default 自身 1 月 −11、新 DF +2（旧分别为 −370、−1555）。
参数效应：DF GPP −7%，Default −3%（stress-deciduous 部分 −7%，树不变）。
累计 NBP 碳收益 10.20 → 10.72 PgC（**+5.1%**），TOTECOSYSC 差值 +4.8%；
只改 DF 为 +8.0%。与 0.5° 结果一致。

**6. 4 km 全部 4 个 SSP（2026-09-24，作业 573706，8 个 38000 s case 全部跑完并核对：
每个 7/7 段读新 paramfile、7/7 段 SUCCESSFUL TERMINATION）。** `Default − DF`
跳变 / 起伏（gC m⁻² yr⁻¹）：

| | 两边 36000 | 只改 DF | 两边 38000 |
|---|---|---|---|
| 1 月 SSP1-1.9 / 2-4.5 / 3-7.0 / 5-8.5 | +796/7, +954/8, +1053/10, +1184/11 | −322…−406 | **−6/7, −6/9, −8/10, −13/13** |
| 全年 | +237/8, +212/7, +250/10, +314/12 | −30…−42 | **+7/6, +3/7, +19/9, +21/11** |

两边 38000 时 4 个 SSP 的 1 月跳变全部落在起伏以内，全年跳变为起伏的 0.4–2 倍。
（SSP2-4.5 的 7 月在两种 38000 配对里都是 −40 左右（起伏 13），两边 36000 时为 +82；
7 月不涉及休眠，与阈值无关。）参数效应：DF GPP −7% 到 −8%，Default −3% 到 −4%
（stress-deciduous 部分 −7% 到 −8%）。累计 NBP 碳收益（2024–2100）相对两边 36000：

| | 两边 36000（PgC） | 只改 DF | **两边 38000** |
|---|---|---|---|
| SSP1-1.9 | 10.70 | +7.1% | **+4.9%**（11.22） |
| SSP2-4.5 | 11.12 | +8.2% | **+5.7%**（11.75） |
| SSP3-7.0 | 10.30 | +8.1% | **+5.3%**（10.85） |
| SSP5-8.5 | 10.20 | +8.0% | **+5.1%**（10.72） |

2100 年 TOTECOSYSC 差值：两边 38000 为 +4.7% 到 +5.5%。与 0.5°（+5.3% 到 +5.9%）一致。
佛罗里达南部另有几类与本参数无关的 GPP 方块，见
`20260910_seus_rerun/20260923_seus_4km_cds38000/SOUTH_FLORIDA_PATCHES.md`。
