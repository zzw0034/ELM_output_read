...
This is a working analysis and writing plan for the southeastern U.S. forest-carbon manuscript. It summarizes the current discussion; it does not report newly calculated results. Use the final, consistently configured simulation cohort and recompute all numbers before writing quantitative claims. The experiment inventory is in [CASE_MATRIX.md](CASE_MATRIX.md); the broader manuscript plan is in [MANUSCRIPT_BLUEPRINT.md](MANUSCRIPT_BLUEPRINT.md).
...
The Introduction motivates three connected issues: incomplete consideration of soil carbon gains in some forest offset accounting, disturbance risks to the persistence of carbon storage, and coarse spatial information that can obscure local management opportunities. The paper tests the spatial consequences of these issues; it does not quantify issued credits or project-level reversal probabilities. Disagreement among SOC data products is relevant to uncertainty, but is not a separate central research question here.
...
- Use TOTECOSYSC for the primary ecosystem stock benefit. Close the sum of carbon-pool contributions against that total before reporting percentage shares.
...
**Plot:** Matched RF-gain maps at native 4 km, aggregated 4 km, and native 0.5°; one or two predefined regional zooms; an area-weighted cumulative distribution or quantile plot; and a compact within-cell heterogeneity statistic.
...
- Close vegetation, soil, litter, and dead-material contributions against TOTECOSYSC. Report residuals explicitly until the accounting is resolved.
- Examine NEP, land-use/product-pool flux, and complete fire loss in the NBP budget. Use spatial strata or maps to connect regional flux findings to local benefit patterns.
...
**Plot:** A Default stock-pool composition panel; signed RF/RH pool contributions; spatial distributions of vegetation and SOC gains; and a concise NBP process contribution panel. Move detailed pool accounting to the supplement if needed.
...
State the relevant model/input constraints: differing 0.5° and 4 km configurations, effective resolution of fire and other drivers, observational product uncertainty, carbon-pool accounting, simplified forest structure and intervention definitions, and processes or market feedbacks outside ELM. Distinguish modeled additional ecosystem storage from creditable offsets.

## 6. Priority work order

1. Freeze the actual case/version mapping, units, masks, periods, and carbon-stock definition.
2. Complete Figure 1's common-support and within-cell evaluations, including the ESA-CCI provenance checks.
3. Recompute Figure 2's paired benefits and Figure 3's spatial distributions from the same cohort.
4. Close Figure 4's carbon-pool accounting and process budget.
